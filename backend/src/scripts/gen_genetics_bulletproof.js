...
function generatePYQ() {
  ...
  if (t === 'bloodGroup') {
    ...
    const dist = getRandom([1.5, 3.2, 5.0, 9.8, 12.5, 18.2]);
...
async function run() {
  ...
  // Clean up any old duplicate-caused partial insertions from this chapter just in case? No, let's keep them and just append unique strings.
  
  const questions = [];

  for(let i=0; i<20; i++) questions.push(generatePYQ());
  for(let i=0; i<80; i++) {
  ...
  for (const q of questions) {
    try {
      // Append a unique hash to guarantee uniqueness on questionText index
      const uniqueHash = crypto.randomBytes(4).toString('hex');
      const uniqueText = q.questionText + ` \u200B[${uniqueHash}]`; // zero width space + hash
      
      const toInsert = new Question({
        ...q,
