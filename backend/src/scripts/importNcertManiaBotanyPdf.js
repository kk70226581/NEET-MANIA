...
const PDF_PATH = process.env.NCERT_MANIA_PDF_PATH
  || 'C:/Users/karan/Downloads/botany-neet-ncert-line-by-line-ncert-line-by_compress_compressed.pdf';
const RENDERER = process.env.PDF_RENDERER_PATH
  || 'C:/poppler-26.02.0/Library/bin/pdftoppm.exe';
const OCR_CACHE_DIR = path.join(process.cwd(), '.cache', 'ncert-mania-botany-ocr');
...
const CHAPTER_ALIASES = [
  ['The Living World', /living world/i],
  ['Biological Classification', /biological classification/i],
  ['Plant Kingdom', /plant kingdom/i],
  ['Morphology of Flowering Plants', /morphology of flowering plants/i],
  ['Anatomy of Flowering Plants', /anatomy of flowering plants/i],
  ['Cell: The Unit of Life', /cell[:\s-]+the unit of life/i],
  ['Biomolecules', /\bbiomolecules\b/i],
  ['Cell Cycle and Cell Division', /cell cycle.*cell division/i],
  ['Transport in Plants', /transport in plants/i],
  ['Mineral Nutrition', /mineral nutrition/i],
  ['Photosynthesis in Higher Plants', /photosynthesis in higher plants/i],
  ['Respiration in Plants', /respiration in plants/i],
  ['Plant Growth and Development', /plant growth and development/i],
  ['Sexual Reproduction in Flowering Plants', /sexual reproduction in flowering plants/i],
  ['Organisms and Populations', /organisms and populations/i],
  ['Ecosystem', /\becosystem\b/i],
  ['Biodiversity and Conservation', /biodiversity(?:\s+and\s+conservation)?/i]
];
...
function cleanText(text = '') {
  return String(text)
    ...
    .filter((line) => {
      ...
      if (/^botany$/i.test(line)) return false;
...
function detectChapter(text, fallback = 'Botany NCERT Practice') {
...
function parseQuestionBlock(block, chapter, answer, sourcePage, documentHash) {
  ...
  return {
    questionId: `ncert-mania-botany-${questionHash}`,
    ...
    sourceDetails: {
      ...
      testName: 'NCERT Mania Botany Line-by-Line'
    },
    ...
    ncertReference: {
      class: ['The Living World', 'Biological Classification', 'Plant Kingdom', 'Morphology of Flowering Plants', 'Anatomy of Flowering Plants', 'Cell: The Unit of Life', 'Biomolecules', 'Cell Cycle and Cell Division'].includes(chapter) ? '11' : '12',
      book: 'NCERT Biology',
      chapter,
      topic: chapter,
    ...
    qualityAudit: {
      ...
      ambiguityScore: 76,
      notes: [
        'OCR extracted from user-provided NCERT line-by-line Botany PDF.',
    ...
    pyq: { isPYQ: false },
    pyqDetails: {
      classLevel: ['The Living World', 'Biological Classification', 'Plant Kingdom', 'Morphology of Flowering Plants', 'Anatomy of Flowering Plants', 'Cell: The Unit of Life', 'Biomolecules', 'Cell Cycle and Cell Division'].includes(chapter) ? '11' : '12',
      unit: 'Botany',
      ncertBased: true,
    ...
    isVerified: true,
    isPublished: true,
    tags: ['ncert-mania', 'botany', 'line-by-line', 'ncert-based', chapter.toLowerCase().replace(/[^a-z0-9]+/g, '-')],
    keywords: [...new Set([chapter, 'NCERT', 'Botany', 'NEET', ...rawQuestion.split(/\W+/).filter((word) => word.length > 5).slice(0, 8)])]
  };
}
...
async function buildQuestions() {
  ...
  let currentChapter = 'Botany NCERT Practice';
...
async function main() {
  ...
  console.log(`Prepared ${questions.length} NCERT Mania questions`);
  ...
  console.log(`Student-visible NCERT Mania questions: ${visible}`);
