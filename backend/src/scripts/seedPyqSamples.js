...
const templates = [
  (year, index) => {
    ...
    return { subject: 'physics', chapter: 'Units and Measurements', topic: 'Dimensional analysis', unit: 'Physics and Measurement', difficulty: 'easy', type: 'numerical', statement: `A practice cart for the ${year} archive moves uniformly at ${speed} m s⁻¹ for ${time} s. What distance does it cover?`, ...rotateAnswer(`${result} m`, [`${result + speed} m`, `${Math.abs(result - speed)} m`, `${speed / time} m`], index % 4), solution: `For uniform motion, distance = speed × time = ${speed} × ${time} = ${result} m.`, formula: 's = vt' };
  },
  (year, index) => {
    const mass = 2 + (index % 4);
    const acceleration = 3 + (index % 3);
    const force = mass * acceleration;
    return { subject: 'physics', chapter: 'Laws of Motion', topic: 'Newton’s second law', unit: 'Mechanics', difficulty: 'medium', type: 'numerical', statement: `In an original ${year} practice scenario, a ${mass} kg block has acceleration ${acceleration} m s⁻². What is the net force?`, ...rotateAnswer(`${force} N`, [`${force + mass} N`, `${force - mass} N`, `${mass / acceleration} N`], (index + 1) % 4), solution: `Newton’s second law gives F = ma = ${mass} × ${acceleration} = ${force} N.`, formula: 'F = ma' };
  },
  (year, index) => {
    const protons = 6 + (index % 4);
    const neutrons = protons + 1 + (index % 2);
    const mass = protons + neutrons;
    return { subject: 'chemistry', chapter: 'Structure of Atom', topic: 'Atomic number and mass number', unit: 'Physical Chemistry', difficulty: 'easy', type: 'numerical', statement: `An electrically neutral atom in the ${year} sample set contains ${protons} protons and ${neutrons} neutrons. Its mass number is:`, ...rotateAnswer(String(mass), [String(protons), String(neutrons), String(mass + protons)], (index + 2) % 4), solution: `Mass number equals protons plus neutrons: ${protons} + ${neutrons} = ${mass}.`, formula: 'A = Z + N' };
  },
  (year, index) => {
    const exponent = 2 + (index % 3);
    return { subject: 'chemistry', chapter: 'Equilibrium', topic: 'Ionic equilibrium', unit: 'Physical Chemistry', difficulty: 'medium', type: 'numerical', statement: `For an original ${year} archive problem, a strong monoprotic acid has concentration 10⁻${exponent} mol L⁻¹. Its pH is:`, ...rotateAnswer(String(exponent), [String(exponent + 1), String(Math.max(0, exponent - 1)), String(14 - exponent)], (index + 3) % 4), solution: `Complete dissociation gives [H⁺] = 10⁻${exponent}; therefore pH = −log[H⁺] = ${exponent}.`, formula: 'pH = −log[H⁺]' };
  },
  (year, index) => ({ subject: 'botany', chapter: 'Photosynthesis in Higher Plants', topic: 'C4 pathway', unit: 'Plant Physiology', difficulty: 'hard', type: 'statement_based', statement: `During a ${year} original practice discussion, which feature correctly distinguishes the C₄ pathway?`, ...rotateAnswer('Initial CO₂ fixation occurs in mesophyll cells through PEP carboxylase.', ['RuBisCO performs the initial fixation in mesophyll cytoplasm.', 'Photorespiration is greater than in comparable C₃ plants.', 'The first stable product is a three-carbon compound.'], index % 4), solution: 'C₄ plants initially fix CO₂ with PEP carboxylase in mesophyll cells, producing a four-carbon acid; the other statements describe C₃ behaviour or reverse the comparison.', formula: 'PEP + HCO₃⁻ → oxaloacetate' }),
  (year, index) => ({ subject: 'botany', chapter: 'Cell: The Unit of Life', topic: 'Cell organelles', unit: 'Cell Biology', difficulty: 'medium', type: 'mcq', statement: `A cell observed in an original ${year} practice slide has cristae inside a double-membrane organelle. Which function is most directly associated with it?`, ...rotateAnswer('Aerobic ATP production', ['Packaging proteins for secretion', 'Hydrolytic digestion at acidic pH', 'Synthesis of ribosomal RNA in the nucleolus'], (index + 1) % 4), solution: 'Cristae are folds of the inner mitochondrial membrane and house the electron-transport machinery used in oxidative phosphorylation.', formula: 'ADP + Pi → ATP' }),
  (year, index) => ({ subject: 'zoology', chapter: 'Human Reproduction', topic: 'Gametogenesis', unit: 'Reproduction', difficulty: 'medium', type: 'mcq', statement: `In the original ${year} practice archive, a primary spermatocyte is followed through meiosis I. Each daughter cell normally contains:`, ...rotateAnswer('23 chromosomes, each still having two chromatids', ['46 single-chromatid chromosomes', '23 single-chromatid chromosomes', '46 chromosomes, each with two chromatids'], (index + 2) % 4), solution: 'Meiosis I separates homologous chromosomes. Each secondary spermatocyte is haploid with 23 duplicated chromosomes; sister chromatids separate only in meiosis II.', formula: '2n → n after meiosis I' }),
  (year, index) => {
    const q = Number((0.1 + (index % 4) * 0.1).toFixed(1));
    const affected = Math.round(q * q * 100);
    return { subject: 'zoology', chapter: 'Evolution', topic: 'Hardy-Weinberg equilibrium', unit: 'Genetics and Evolution', difficulty: 'hard', type: 'numerical', statement: `In an original ${year} population exercise, the recessive allele frequency is ${q.toFixed(1)}. Under Hardy-Weinberg equilibrium, what percentage shows the recessive phenotype?`, ...rotateAnswer(`${affected}%`, [`${Math.round(q * 100)}%`, `${Math.round((1 - q) * 100)}%`, `${Math.round(2 * q * (1 - q) * 100)}%`], (index + 3) % 4), solution: `The recessive phenotype frequency is q² = ${q.toFixed(1)}² = ${(q * q).toFixed(2)}, or ${affected}%.`, formula: 'p² + 2pq + q² = 1' };
  }
];
...
async function main() {
  ...
  const existing = new Set((await Question.find({ questionId: { $in: [...ids] } }).select('questionId').lean()).map((item) => item.questionId));
  const missing = documents.filter((item) => !existing.has(item.questionId));
  if (missing.length) await Question.insertMany(missing, { ordered: true });
  await PyqAnalyticsSnapshot.deleteMany({});
  console.log(JSON.stringify({ inserted: missing.length, alreadyPresent: existing.size, totalSamples: await Question.countDocuments({ 'pyqDetails.legalStatus': 'original_sample' }) }, null, 2));
}
