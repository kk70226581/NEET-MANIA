...
async function run() {
  try {
    const rawFilePath = path.join(__dirname, 'motion_2d_raw.txt');
    ...
    for (let index = 0; index < blocks.length; index++) {
      ...
      parsedQuestions.push({
        questionId: `motion-2d-practice-${parsedQuestions.length + 1}-${Math.random().toString(36).substring(2, 7)}`,
        ...
        topic: ncertTopic || 'Motion in Two Dimensions',
        difficulty: difficulty,
        explanation: { text: explanation || 'Refer to NCERT Motion in a Plane.' },
        assertion: {
        ...
        source: 'custom',
        tags: ['practice', 'motion-in-a-plane', 'physics', 'ncert-aligned'],
        isPublished: true,
        isVerified: true,
        inSyllabus: true,
