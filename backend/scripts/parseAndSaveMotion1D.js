...
async function run() {
  try {
    const rawFilePath = path.join(__dirname, 'motion_1d_raw.txt');
    ...
    for (let index = 0; index < blocks.length; index++) {
      ...
      parsedQuestions.push({
        questionId: `motion-1d-practice-${parsedQuestions.length + 1}-${Math.random().toString(36).substring(2, 7)}`,
        ...
        topic: ncertTopic || 'Motion in One Dimension',
        difficulty: difficulty,
        explanation: { text: explanation || 'Refer to NCERT Motion in a Straight Line.' },
        assertion: {
        ...
        source: 'custom',
        tags: ['practice', 'motion-in-a-straight-line', 'physics', 'ncert-aligned'],
        isPublished: true,
        isVerified: true,
        inSyllabus: true,
