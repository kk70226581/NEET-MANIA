...
try {
  ...
  if (lastUserLine) {
    ...
    // We want to extract everything between the start of the user request and the end
    const scriptContent = lastUserLine.content;
    const outputPath = 'c:\\Users\\karan\\OneDrive\\Desktop\\NEET\\NEETP\\backend\\scripts\\raw_genetics_script.py';
    fs.writeFileSync(outputPath, scriptContent);
    console.log(`Saved raw python script to ${outputPath}`);
  } else {
