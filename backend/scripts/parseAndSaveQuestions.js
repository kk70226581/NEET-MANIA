    ...
    for (let index = 0; index < blocks.length; index++) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (questionText) {
          ...
          // Strip leading number index (e.g. "128. ")
          questionText = line.replace(/^\d+\.\s*/, '');
        }
      }

      // Final check: if we didn't parse options or correctAnswer, something went wrong
      if (!options.A || !options.B || !options.C || !options.D || !correctAnswer) {
        console.warn(`Warning: block ${index + 1} did not parse completely. Line 1: "${lines[0]}"`);
        continue;
      }
      ...
      parsedQuestions.push({
        questionId: `mole-concept-practice-${index + 1}-${Math.random().toString(36).substring(2, 7)}`,
        ...
        topic: ncertTopic || 'Mole Concept',
        difficulty: difficulty,
        explanation: { text: explanation || 'Refer to NCERT Mole Concept.' },
        assertion: {
        ...
        source: 'custom',
        tags: ['practice', 'mole-concept', 'chemistry', 'ncert-aligned'],
        isPublished: true,
        isVerified: true
      });
    }

    console.log(`Parsed ${parsedQuestions.length} valid questions successfully.`);
