...
async function run() {
  try {
    const rawFilePath = path.join(__dirname, 'molecular_basis_raw.txt');
    ...
    for (let index = 0; index < blocks.length; index++) {
      ...
      parsedQuestions.push({
        questionId: `molecular-basis-practice-${parsedQuestions.length + 1}-${Math.random().toString(36).substring(2, 7)}`,
        ...
        topic: ncertTopic || 'Molecular Basis of Inheritance',
        difficulty: difficulty,
        explanation: { text: explanation || 'Refer to NCERT Molecular Basis of Inheritance.' },
        assertion: {
        ...
        source: 'custom',
        tags: ['practice', 'molecular-basis', 'biology', 'ncert-aligned'],
        isPublished: true,
        isVerified: true,
        inSyllabus: true,
