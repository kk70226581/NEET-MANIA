...
async function run() {
  try {
    const rawFilePath = path.join(__dirname, 'units_and_measurements_raw.txt');
    ...
    for (let index = 0; index < blocks.length; index++) {
        ...
        if (line.includes('Answer:') && line.includes('|')) {
          ...
          for (const part of parts) {
            if (part.startsWith('Answer:')) {
              ...
              ncertTopic = part.replace('Topic:', '').trim();
            }
          }
          continue;
        }
      ...
      parsedQuestions.push({
        questionId: `units-measurements-practice-${parsedQuestions.length + 1}-${Math.random().toString(36).substring(2, 7)}`,
        ...
        topic: ncertTopic || 'Units and Measurements',
        difficulty: difficulty,
        explanation: { text: explanation || 'Refer to NCERT Units and Measurements.' },
        assertion: {
        ...
        source: 'custom',
        tags: ['practice', 'units-and-measurements', 'physics', 'ncert-aligned'],
        isPublished: true,
        isVerified: true,
        inSyllabus: true,
