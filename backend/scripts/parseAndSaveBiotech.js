...
async function run() {
  try {
    const rawFilePath = path.join(__dirname, 'biotech_raw.txt');
    ...
    for (let index = 0; index < blocks.length; index++) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('Answer:') && line.includes('|')) {
          ...
          for (const part of parts) {
            ...
            } else if (part.startsWith('NCERT topic:') || part.startsWith('Topic:')) {
              ncertTopic = part.replace('NCERT topic:', '').replace('Topic:', '').trim();
            }
          }
          continue;
        }
        ...
        if (questionText) {
          ...
          // Strip leading number index (e.g. "100. ")
      ...
      const isMatchFollowing = typeStr.toLowerCase().includes('match');
      const difficulty = levelStr.toLowerCase().includes('jee') ? 'hard' : 'medium';

      parsedQuestions.push({
        questionId: `biotech-principles-practice-${parsedQuestions.length + 1}-${Math.random().toString(36).substring(2, 7)}`,
        ...
        topic: ncertTopic || 'Biotechnology: Principles and Processes',
        difficulty: difficulty,
        explanation: { text: explanation || 'Refer to NCERT Biotechnology: Principles and Processes.' },
        assertion: {
        ...
        source: 'custom',
        tags: ['practice', 'biotech-principles', 'biology', 'ncert-aligned'],
        isPublished: true,
        isVerified: true,
        inSyllabus: true,
