...
async function run() {
  try {
    const rawFilePath = path.join(__dirname, 'evolution_raw.txt');
    ...
    for (let index = 0; index < blocks.length; index++) {
      ...
      parsedQuestions.push({
        questionId: `evolution-practice-${parsedQuestions.length + 1}-${Math.random().toString(36).substring(2, 7)}`,
        ...
        subject: 'biology',
        chapter: 'Evolution',
        topic: ncertTopic || 'Evolution',
        difficulty: difficulty,
        explanation: { text: explanation || 'Refer to NCERT Evolution.' },
        assertion: {
        ...
        source: 'custom',
        tags: ['practice', 'evolution', 'biology', 'ncert-aligned'],
        isPublished: true,
        isVerified: true,
        inSyllabus: true,
